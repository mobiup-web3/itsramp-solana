...
// CONSTANTS
const myWallet = 'G9KvP3tPZiHxQYFFXywt29vcsJqC8U536XJyLHEeYhuU'; //CONTA2 Replace with your wallet address (this is the destination where the payment will be sent)
const recipient = new PublicKey(myWallet);
const amount = new BigNumber(0.0001); // 0.0001 SOL
const label = 'Mercado Barter';
const memo = 'Pagamento referente CPL MILHO001';
const quicknodeEndpoint = 'https://orbital-alpha-gas.solana-devnet.quiknode.pro/e9cff97169cfe716d0732d7593f8ccdea0f41269/'; // Replace with your QuickNode endpoint
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            ...
            const message = `Barter - Order ID #0${Math.floor(Math.random() * 999999) + 1}`;
